...
Class Box2DgfxTest Extends Window
	...
	Field myCollisionCallback:CollisionCallback
	Field radarFixture:b2Fixture
	...
	'------- Initialising the world with its gravity
		...
		world=mx2b2dJson.Loadb2dJson("asset::sensorbalance.json")
...
'		mx2b2dJson.testNew()
	...
	'----- debugdrawer init and link---------------------------------------------------------------------------------------------
		...
		'Getting the static body and assigning it to staticBody in order to create the mouse joint
		staticBody=world.GetBodyList()
		While staticBody.GetType()<>b2BodyType.b2_staticBody
			staticBody=staticBody.GetNext()
		Wend
		'Getting the unique kinetic body and assigning it to pass it to the CollisionCallback (the radar)
		Local kineBody:=world.GetBodyList()
		While kineBody.GetType()<>b2BodyType.b2_kinematicBody
			kineBody=kineBody.GetNext()
		Wend
		radarFixture=kineBody.GetFixtureList()
		myCollisionCallback=New CollisionCallback (radarFixture)
		world.SetContactListener(myCollisionCallback)
	...
	Method OnRender( canvas:Canvas ) Override
		...
		canvas.DrawText("grab objects with you mouse and pass them in the radar to see senor prints in console.",15,15)
...
Class CollisionCallback Extends b2ContactListener

	Field fixt:b2Fixture

	Method New(fix:b2Fixture)
		fixt=fix
	End

	Method BeginContact( contact:b2Contact ) Override
		If contact.GetFixtureA()=fixt Or contact.GetFixtureB()=fixt
			Print "Begin radar Contact"
		End
	End
	
	Method EndContact (contact:b2Contact) Override
		If contact.GetFixtureA()=fixt Or contact.GetFixtureB()=fixt
			Print "End radar Contact"
		End
	End	

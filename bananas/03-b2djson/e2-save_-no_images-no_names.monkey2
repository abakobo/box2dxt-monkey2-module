...
Class Box2DgfxTest Extends Window
	...
	Field translate:=New Vec2f(430,640) 
	Field zoom:=3.0
	...
	'----- debugdrawer init and link---------------------------------------------------------------------------------------------
		DDrawer=New b2DebugDraw(physScale,True) 'this one must be a field or a global 
		...
		If Keyboard.KeyPressed(Key.Space)
		 ...
		 If Mouse.ButtonPressed(MouseButton.Left)
			...
			If (callback.q_fixture)
				...
				jdMouse.maxForce=1000.0*bodyForMouseJoint.GetMass()

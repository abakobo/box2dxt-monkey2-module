...
namespace p2bayazit
{
	...
	static Point operator +(const Point &a, const Point &b) { return Point(a.x + b.x, a.y + b.y); }
	static Point operator /(const Point &a, float x) { return Point(a.x / x, a.y / x); }
	...
	static bbArray<bbArray<p2bayazit::Point>> BayazitConvexor(bbArray<p2bayazit::Point> inArr){

		p2bayazit::Polygon poly;
	
		for(int i=0;i<inArr.length();i++) {
			poly.push_back(inArr[i]);
		}
		...
		copy(result.begin(),result.end(),resultarr);
	 	...
	 	for (int pi=0;pi<result.size();pi++) {
				...
				returnArr[pi]=tempArr;
				tempArr.release();
				returnArr[pi].retain();
		}
		returnArr.release();
		for (int pi=0;pi<result.size();pi++) {
			returnArr[pi].release();
		}
	 	delete [] resultarr;
	    return returnArr;
	}
